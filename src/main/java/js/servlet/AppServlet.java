 ...
 * <p>
 * Also this base class takes care to initialize logger context, {@link #logContext} with remote address of current processing
 * request. This diagnostic context data is bound to current thread and accessible to all log messages generated by methods
 * involved in request processing, no matter nesting level.
 * 
...
public abstract class AppServlet extends HttpServlet {
	...
	/** Logger for the time elapsed on request processing; accessible with name <code>app.servlet.request</code>. */
	private static final Log requestLog = LogFactory.getLog("app.servlet.request");
	...
	protected void service(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException, ServletException {
		// push remote address of the requested processed by this thread to logger diagnostic context
		...
		try {
			...
			requestLog.debug("%s %s processed in %d msec.", httpRequest.getMethod(), context.getRequestURL(), System.currentTimeMillis() - start);
			logContext.pop();
			context.detach();
		}
	}
