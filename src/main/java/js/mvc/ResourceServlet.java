 ...
 * request path is {@link ManagedClassSPI#getRequestPath()}. A resource is a managed method from a controller that returns a
 * {@link Resource} and <code>resource</code> from request path is {@link ManagedMethodSPI#getRequestPath()}.
 * 
 * <pre>
 ...
 * container has one defined, see {@link ContainerSPI#getLoginPage()}. If there is not login page performs servlet container
...
public class ResourceServlet extends AppServlet {
	 ...
	 * {@link #key(ManagedMethodSPI)} based on controller and resource method request paths and is paired with retrieval key -
	 ...
	 * {@link ManagedClassSPI#getRequestPath()} and resource path is managed method request path,
	 * {@link ManagedMethodSPI#getRequestPath()}.
	 * 
	 * <pre>
