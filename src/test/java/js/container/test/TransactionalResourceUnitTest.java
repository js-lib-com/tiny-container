...
import js.test.stub.TransactionStub;
...
public class TransactionalResourceUnitTest {
	...
	public void threadSessionObject() throws InterruptedException {
		...
		assertEquals(session, transactionalResource.getSession());
		...
		Thread thread = new Thread(new Runnable() {
			@Override
			public void run() {
				threadSession.session = transactionalResource.getSession();
			}
		});
		thread.start();
		thread.join();
	...
	private static TransactionalResource createTransactionalResource(AppFactory factory) {
		try {
			return Classes.newInstance("js.container.TransactionalResourceImpl", factory);
